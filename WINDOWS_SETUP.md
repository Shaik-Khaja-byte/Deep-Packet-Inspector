...
```cmd
dpi_engine.exe samples\sample_capture.pcap output.pcap
```
...
```bash
./dpi_engine samples/sample_capture.pcap output.pcap
```
...
**Fix:** Make sure samples\sample_capture.pcap exists:
```cmd
dir samples\sample_capture.pcap
```
...
2. Try a different output filename:
   ```cmd
   dpi_engine.exe samples\sample_capture.pcap result.pcap
   ```
...
7. Save as a local capture such as `capture_local.pcap`

8. Run DPI engine on it:
   ```cmd
   dpi_engine.exe capture_local.pcap filtered.pcap
   ```
