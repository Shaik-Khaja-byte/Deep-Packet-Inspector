...
private:
    ...
    void run()
    {
        while (running_)
        {
            ...
            // Track both directions of the same connection in one flow record.
            FiveTuple flow_key = canonicalizeFiveTuple(pkt.tuple);
    ...
    void classifyFlow(Packet &pkt, FlowEntry &flow)
    {
        ...
        // Try QUIC/HTTP3 on UDP/443.
        if (pkt.tuple.protocol == 17 && (pkt.tuple.dst_port == 443 || pkt.tuple.src_port == 443) && pkt.payload_length > 0)
        {
            const uint8_t *payload = pkt.data.data() + pkt.payload_offset;
