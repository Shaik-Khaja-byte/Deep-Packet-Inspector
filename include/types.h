...
namespace DPI
{
    ...
    // Return a direction-independent key for a flow.
    // This keeps both directions of the same connection on the same hash bucket.
    ...
    struct FiveTupleHash
    {
        ...
        {
            const FiveTuple normalized = canonicalizeFiveTuple(tuple);
            // Simple but effective hash combining all fields
            size_t h = 0;
            h ^= std::hash<uint32_t>{}(normalized.src_ip) + 0x9e3779b9 + (h << 6) + (h >> 2);
            h ^= std::hash<uint32_t>{}(normalized.dst_ip) + 0x9e3779b9 + (h << 6) + (h >> 2);
            h ^= std::hash<uint16_t>{}(normalized.src_port) + 0x9e3779b9 + (h << 6) + (h >> 2);
            h ^= std::hash<uint16_t>{}(normalized.dst_port) + 0x9e3779b9 + (h << 6) + (h >> 2);
            h ^= std::hash<uint8_t>{}(normalized.protocol) + 0x9e3779b9 + (h << 6) + (h >> 2);
            return h;
        }
    };
